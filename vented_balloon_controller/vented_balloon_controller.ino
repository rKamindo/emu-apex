...
const int REQUIRED_AGREEMENTS = 10;  // number of agreements in proposed state
                                     // required to change flight state
...
// current flight state, initialized to fast ascent, test what happens when this
// is set to FLOAT
...
void setup() {
  Serial.begin(9600);
  ventServo.attach(VENT_SERVO_PIN, 1000,
                   2000);  // adjust with servo parameters as needed
  ventServo.write(VENT_CLOSED_POSITION);  // ensure the vent is initially closed
...
FlightState proposeState(float ascentRaate) {
    ...
    switch (currentState) {
      ...
      case FLOAT:
        if (currentAltitude >= TARGET_ALTITUDE) {
          if (ventOpen) {  // Close vent if it's open and ascent rate indicates
                           // descent
            closeVent();
            ventOpen = false;
          ...
          if (!floatComplete && floatTimerStarted &&
              (millis() - floatTimerStart >= FLOAT_DURATION)) {
            if (!ventOpen) {  // Open the vent only if it's not already open
              openVent();
              ventOpen = true;
              floatComplete = true;
              Serial.println(
                  "Float duration exceeded. Opening vent to release all gas.");
            }
          }
        }
        break;
